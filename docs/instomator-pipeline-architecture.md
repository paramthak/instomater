...
|---|---|---|---|---|
| 1 | Session start | Person/topic name plus optional context in one text input | `script_prompt.txt`, `script_v1.json`, script card | User approves/changes/restores |
| 2 | Photo upload | Uploaded reference photo after script approval | `uploaded_photo.<ext>` | User confirms photo |
| 3 | Voiceover | Approved script, selected gender/custom voice ID and speed | `voiceover_v1.mp3`, voiceover card | User approves/changes voice |
| 4 | Forced alignment | Voiceover MP3 plus clean script text | `alignment.json` with word timestamps | Automatic next step |
| 5 | Storyboard | Script and alignment | `storyboard_v1.json`, storyboard card | User approves/changes/retries |
| 6 | Image generation | Storyboard and uploaded photo | Two unique keyframe PNGs per clip in `images/` | User approves/regenerates each image |
| 7 | Video generation | Approved unique start/end images, storyboard scene, video prompt | Prompt in `video_prompts/`, MP4 in `videos/` | User approves/regenerates each clip |
| 8 | Assembly | Approved videos, voiceover, alignment, script | `final/reel_v1.mp4`, FFmpeg logs | Session locks on click |
| 9 | Final review | Final MP4 and cost ledger | Review page with final reel, download, and costs | End state |
...
```text
initial user input
  -> becomes the script generation input
script
  -> becomes voiceover
voiceover + script
  -> becomes alignment
...
|---|---|---|
| Script | Write the spoken reel | Use the uploaded script-writer skill and the raw initial user input. Keep it short, simple, relatable, and education-abroad focused. |
...
script_prompt.txt          raw initial user input
...
The final reel quality depends on clean handoffs. The initial user input feeds the script, the script feeds the voiceover, the voiceover creates real timing, the timing shapes the storyboard, the storyboard guides images/videos, and approved clips give assembly reliable inputs.
